...
parts.push(
  ...
  + `   - **行頭に ★ を付けるのは「今後すべての作業に効く普遍ルール」だけ**（事業判断・境界・恒久方針）。\n`
  + `     その作業限りの実装細部（命名・慣習合わせ・手順の選択）には付けない = 記録のみで人に聞かない。\n`
  + `     迷ったら付けない。人の時間を使う価値があるものだけに ★ を使う\n`
...
try {
  ...
  if (dn + pn > 0) {
    ...
    parts.push(
      ...
      + `「決めてほしいことが ${dn + pn} 件あります。1 回答えるだけで、AI は次からずっとその通りに動きます（『決める』と言えばいつでも）」\n`
      + `人間が「決める」「決めます」「やる」（または「裁定して」）と宣言した時に、1 件ずつ簡単な三択で提示する。催促しない。\n`
      + `トリガーは人間の一人称宣言。「決めて」という依頼形は決定の AI への移譲に読めるため使わない。`,
    );
  }
} catch {}
