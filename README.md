...
業界の名作 (Toyota Production System / Site Reliability Engineering / Domain-Driven Design 等) を一本の幹 **「顧客の安全 > 進捗」** で統合し、AI agent が危険な操作だけを手前で止め、仕事全体は止めずに安全な経路へ流せるよう実装してある。
...
人間レビュー / CI ガードレールだけでは追いつかない。AI 自身が **危険な操作は止め、仕事全体は止めずに安全な経路 (rework lane) へ変える装置** が要る。
