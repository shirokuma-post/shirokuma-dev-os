...
- Codex 側は一切未検証 (codex CLI 未導入 = §4.2 参照)。
...
| cross-platform | 3/10 | 3/10 | 8/10 | Codex 側 0 (CLI 未導入・全記述が未検証仮説) |
...
│   └── marketplace.json     # 実在
├── .codex-plugin/
│   └── plugin.json          # ⚠ blocked (下記 4.2 Codex)
├── skills/
...
> ⚠ **blocked (2026-07-11 実測)**: codex CLI はこの Mac に未インストール (`which codex` = not found)。以下の記述は**未検証仮説**であり、Codex CLI 導入後に実測してから着手する。事実として扱わない。

- `.codex-plugin/plugin.json` (仮説)
- strict semver、author、description、interface metadata、`skills: "./skills/"`を持つ (仮説)。
- unsupported fieldを入れない。
- personal marketplaceはlocal development専用、repo/team marketplaceは明示依頼時のみ作る (仮説)。
- cachebuster更新はhelper script経由とし、marketplaceを手編集しない (仮説)。
...
- Codex pluginへ`validate_plugin.py` — ⚠ **blocked: codex CLI 未導入 (2026-07-11 実測)・validator の存在自体が未検証仮説。CLI 導入後に実測してから着手**
...
- Claude CodeとCodexの両方で同じtask setを実行する (Codex 側は ⚠ CLI 導入後)。
...
- `1.4.x`: 6 skill trigger / behavior eval 整備 + description 絞り込み (§7.2/7.3)
- `1.5.x`: agents / commands / doctor.mjs (§6 W7/W8)
- `1.6.x`: universal core / stack pack 分離 (§5 W4)
- `1.7.x`: cross-platform (Codex 実測) + dogfood 3種 = public alpha 相当
- `2.0.0`: 外部利用証拠と互換方針を満たしたstable (初版 1.0.0 gate 相当)

version番号はgate達成を表し、cache更新目的で乱用しない。Codex local iterationのcachebuster運用は ⚠ CLI 導入後に実測してから定める。
...
- Codex: ⚠ **blocked: codex CLI 未導入 (2026-07-11 実測)。personal local marketplace 記述は未検証仮説・CLI 導入後に実測してから着手**
...
- Codex向け公開plugin source (⚠ 同上 blocked)
...
- dual manifestを作る → Claude 側**達成**・Codex 側 ⚠ blocked (§4.2)。
...
- Claude / Codex local install smokeを作る → Claude 側 live E2E PASS 済・Codex 側 ⚠ blocked。

Gate (条件分岐へ改訂 2026-07-11):

- ~~Claude CodeとCodexが同じ4 skillを発見する~~ → **Claude Code 側: 6 skill discovery を実測する** (5 skill 時点の E2E は PASS 済・guardrails 込み再実測が残)。**Codex 側: CLI 導入後に同一 task set で実測** (それまで gate 対象外・blocked を明記)。
- install / list / invoke / uninstall / reinstallが通る (install/invoke は PASS 済・uninstall/reinstall 実測が残)。
