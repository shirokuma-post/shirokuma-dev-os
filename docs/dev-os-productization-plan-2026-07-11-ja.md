...
> **2026-07-11 第 2 改訂**: 外部レビュー (72/100) 全採用 Wave 反映 (§14 に採用記録・対応 commit 対照表)。
...
「現在(初版)」列 = snapshot 実測に基づく初版採点。「上流実測」列 = 上流正典 v1.2.2〜1.3.0 の live 実測に基づく再採点。「Wave後」列 = 外部レビュー全採用 Wave (v1.5.0・§14) 後の再採点 (各行の根拠 = 「‖ Wave後:」以降の 1 行)。

| 軸 | 現在(初版) | 上流実測 | Wave後 | 目標 | 再採点根拠 (2026-07-11 実測) |
|---|---:|---:|---:|---:|---|
| 思想・一貫性 | 8.5/10 | 8.5/10 | 8.5/10 | 9/10 | 変わらず (本文は snapshot と md5 ほぼ一致) ‖ Wave後: 据え置き (思想節は非接触・routing は運用則の精密化) |
| skill本文 | 7.5/10 | 7.5/10 | 8/10 | 8.5/10 | 変わらず (§5 の改善は未着手) ‖ Wave後: W2 routing (344c5b1) + W4 pack 分離 (981ac1d) 反映で微増。W1/W3 の description 絞り込みは未 |
| 実務資産 | 8/10 | 8.5/10 | 9/10 | 9/10 | guardrails port 完了で scripts 17 本 + references 7 本が正典入り ‖ Wave後: pack.json ×4 metadata + init preset/検出/dry-run + eval harness/fixture 3 本が資産入り |
| 発火精度 | 4/10 | 4/10 | 4.5/10 | 8.5/10 | eval 0 件のまま = 未検証 (VALIDATION.md 自身が認める) ‖ Wave後: case 80 + harness で計測手段は整備、**実測値は依然ゼロ** = 微増に留める |
| plugin梱包 | 2.5/10 | 8/10 | 8/10 | 9/10 | plugin.json + marketplace.json + LICENSE + CHANGELOG + CI + audit script 実在・validate PASS ‖ Wave後: 据え置き (本 Wave 非対象) |
| 導入・更新 | 2/10 | 7.5/10 | 8/10 | 9/10 | `/plugin install` live E2E PASS 済 (CHANGELOG [1.2.2])・doctor / uninstall 検証は未 ‖ Wave後: init preset/stack検出/dry-run/backup 実走 3 パターン確認 (a1cb651)。doctor / uninstall 検証は依然未 |
| cross-platform | 3/10 | 6.5/10 | 6.5/10 | 8/10 | Codex dual manifest + install/discovery/remove live smoke PASS (2026-07-11・codex-cli 0.144.1)。同一 task set 実走と dogfood は残 ‖ Wave後: 据え置き (本 Wave 非対象) |
| 自動検証 | 4/10 | 5.5/10 | 6.5/10 | 9/10 | self-integrity CI + audit-self-integrity 全 PASS・trigger/behavior eval は 0 件 ‖ Wave後: audit check7 (skill 言及整合・FAIL 検出証明済・bfbb246) + eval 整備。eval 実走 KPI はなし |
| 総合 | 5.5〜6/10 | 6.5〜7/10 | 7〜7.5/10 | 8〜8.5/10 | 梱包・導入は達成済、発火精度と実行可能化が残る ‖ Wave後: 実行可能化 (routing/pack/init) は前進、発火精度は未実測のまま = ここが残りの主戦場 |
...
**status (2026-07-11)**: ✅ 実装済 (344c5b1) — capability/risk-based routing 表・「絶対コードを書かない」→ capability-aware 化・rework lane・worker handoff 6 項目必須化を SKILL.md + CLAUDE.template.md へ反映。発火精度の実測 (§7.2 KPI) は未 = eval 実走待ち。
...
**status (2026-07-11)**: 上流へ port 完了 (6567a82)。以下の改善 (特に universal / stack 分離) は依然有効な TODO。→ **同日追記: universal / stack pack 分離は 981ac1d で実装済** (下記 pack 境界の status 参照)。false-positive / false-negative fixture 整備は未 = 残 TODO。
...
✅ **分離済 (2026-07-11・981ac1d)** — 17 script を `assets/packs/{universal 6, typescript 6, nextjs 1, supabase-postgres 3}` + packs/README へ再配置 (16 file は git R100 = 内容不変)。pack ごとに `pack.json` (id/scope/severity/evidence/remediation/limitations) 付与。INVARIANTS.template は普遍 + pack fragment に分離 (missing 0 を実測確認)。deterministic installer は init.mjs 側で実装 (W7 参照)。
...
**status (2026-07-11)**: ✅ 上記のうち preset (`--preset=minimal/saas`)・`--packs` 明示指定・stack 検出 (根拠付き推奨)・INVARIANTS fragment 合成・`--dry-run`・`--force` = `.bak-<日時>` backup 後上書き (デフォルトは既存 file skip)・placeholder 残数の正直表示 (完了扱いしない) を実装済 (a1cb651・実走 3 パターンで確認済: minimal 33 種 / saas 44 種の placeholder 残数を表示)。対話は TTY 時の stack 検出採用 Y/n 確認のみ (init.mjs L393 実測)。diff preview / rollback receipt は未 = 残 TODO。
...
**status (2026-07-11 同日更新)**: ✅ case 80 件 (positive 48 / negative 20 / overlap 12 = cases.jsonl 集計実測) + resumable harness (`tests/trigger-evals/run-trigger-eval.mjs`・`--calibrate` / `--sample` / `--resume` / `--report` / 20 件超は `--confirm-full` ガード) 整備済 (20216bf)。**pilot / フル 80 case は未実走** — headless `claude -p` がこの Mac で未認証 (Desktop 認証は子 CLI に非伝播・raw log 有)。解除手順 = `claude /login` 後 `--calibrate` → `--sample 15`。下記 KPI の実測値は存在しない。
...
**status (2026-07-11)**: 設計 (tests/behavior-evals/README.md) + fixture 3 本 (scope-discipline / evidence-completion / live-vs-template・各 task + rubric) 整備済 (20216bf)。**実走は残** (trigger eval と同じ auth 待ち)。
...
- `1.5.0`: **外部レビュー全採用 Wave (達成 2026-07-11・§14)** — staff-officer routing (344c5b1) + pack 分離 (981ac1d) + init preset/検出/dry-run (a1cb651) + trigger eval 80 case + harness + behavior eval 設計/fixture (20216bf) + audit check7 (bfbb246)。**eval は「整備済・未実走」= KPI 実測値なし** (auth 待ち・§7.2 status)
- `1.5.x`〜`1.6.x`: eval pilot / フル 80 case 実走 + KPI 初計測 + description 絞り込みの残・agents / commands / doctor.mjs (§6 W8) — ~~universal core / stack pack 分離 (§5 W4)~~ は 1.5.0 で前倒し達成
...
### Phase 2 — Doctrine Precision — **作業は大半整備済・Gate は未通過 (2026-07-11)**

作業:

- §3の正典優先順位を全skill / templateへ反映する。
- trigger descriptionを絞る。
- staff-officerをcapability-awareにする → ✅ 344c5b1 (routing 表 + rework lane + handoff 必須 6 項目)。
- stop / block / reworkの対象を精密化する → ✅ 344c5b1 (§3.4 の工程変更を SKILL.md へ反映)。
- trigger / behavior evalを作る → ✅ 20216bf (trigger 80 case + harness / behavior 設計 + fixture 3 本)。**作成済であって実走済ではない**。

Gate (2026-07-11 時点 = **未通過**。eval 未実走のため KPI 実測値が存在しない):

- trigger KPI達成 → 未 (case / harness 整備済・pilot は `claude /login` 待ち)。
- scope外無断変更 0 → 未計測 (behavior eval 実走待ち)。
- 小タスク過剰orchestration基準達成 → 未計測。
- live factよりtemplateを優先するケース 0 → 未計測 (fixture live-vs-template 用意済)。
...
## 14. 外部レビュー採用記録 (2026-07-11)

外部レビュー (2026-07-11・総合 72/100) の指摘 5 件を全て live で裏取りした上で全採用し、同日 Wave で実装した (= v1.5.0)。

### 14.1 指摘 → 対応 commit 対照表

| # | レビュー指摘 (裏取り結果) | 対応 | commit |
|---|---|---|---|
| 1 | GOVERNANCE 第2層 / init.mjs / README の語調が §3.4「停止ではなく工程変更」等と不整合・skill 言及に guardrails 漏れ (3 箇所 live 確認) | semantic 整合修正 + audit check7 = skill 言及整合検査を追加 (汎用・意図的 FAIL 注入で検出動作を証明済) | bfbb246 |
| 2 | staff-officer「参謀は絶対コードを書かない」が capability 非依存表現のまま・routing 規準なし (SKILL.md live 確認) | capability/risk-based routing 表 (§5 W2 の表を実装)・小タスク直接実装・review NG = rework lane・worker handoff 6 項目必須化 | 344c5b1 |
| 3 | guardrails scripts に universal と TS/Next/Supabase 固有が混在 (scripts/ 17 本 live 確認) | `assets/packs/{universal 6, typescript 6, nextjs 1, supabase-postgres 3}` + packs/README へ分離 (16 file R100 = 内容不変)・pack.json ×4 (id/scope/severity/evidence/remediation/limitations)・INVARIANTS 92 行 = 普遍 72 + fragment 20 (missing 0 証明) | 981ac1d |
| 4 | init.mjs に stack 観測・pack 選択・dry-run・backup がない (§6 W7 TODO のまま = live 確認) | `--preset=minimal/saas`・`--packs`・stack 検出 (根拠付き推奨)・INVARIANTS fragment 合成・`--dry-run`・`--force` backup・placeholder 残数の正直表示 (実走 3 パターン証明済: minimal 33 種 / saas 44 種) | a1cb651 |
| 5 | trigger / behavior eval 0 件 = 発火精度が未検証 (tests/ 不在 = live 確認) | trigger eval 80 case (positive 48 / negative 20 / overlap 12) + resumable harness (`--calibrate`/`--sample`/`--resume`/`--report`/20 件超 `--confirm-full` ガード)・behavior eval 設計 + fixture 3 本 | 20216bf |

### 14.2 採用しなかった点 (1 件)

- **「軽量 / 標準 / 厳格モード」の新機構** — モードという別 axis を足すと routing 規準と二重管理になるため不採用。W2 の capability/risk-based routing 表 1 本に統合した (リスクと独立性が同じ判断を担う = 1事実1正典)。

### 14.3 残 (正直な未完リスト)

- **eval pilot 実走** — headless `claude -p` がこの Mac で未認証 (Desktop 認証は子 CLI に非伝播・raw log 有)。解除 = `claude /login` 後 `--calibrate` → `--sample 15`。
- **behavior eval 実走** — fixture 3 本まで整備・実走は同じ auth 待ち。
- **フル 80 case 実走 + KPI 初計測** — §7.2 KPI (precision/recall ≥ 0.90 等) の実測値は現時点で存在しない。§9 Phase 2 Gate は未通過。
