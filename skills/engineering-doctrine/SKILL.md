...
description: 開発の思考様式ガード (作者専用層・第 1 層 = n=1 実例集)。作者のプロダクト群 (TS/Supabase/Next.js スタック) での実装・修正・調査・デバッグ・レビュー時に常時使う。特に「as any/段階N先送り/別タスク化で逃げそう」「推測でRLS・権限・設定を判定しそう」「個別バグに飛びつき構造を見ていない」「仕組みを1箇所だけ入れて完了にしそう」「重要な依存を緩い指定のまま放置しそう」な瞬間に発動。逃げない・根源・実測・幹から・やり切る・固定する の6規律で点検する普遍原理。Python/Rust 等の非 TS スタックや一般/外部プロジェクトの開発判断では、本 skill ではなく配布版 engineering-doctrine-universal の方を使う。
---
