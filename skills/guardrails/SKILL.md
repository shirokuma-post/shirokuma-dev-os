...
- 実装雛形: `scripts/`（gitleaks・lefthook・CIワークフロー・禁止パターンgrep・anyラチェット・RLS無効/テナント越境SQL・maxDuration/依存ドリフト/死活スモーク・dependabot・CodeQL/Semgrep・semgrepルール(失敗カタログのAST化)・SLO定義）。配線は `scripts/README.md`。コピーして `{{ }}` を埋める。
